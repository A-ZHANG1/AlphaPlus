 ...
 * events. These events are buffered not in heap space,
 ...
 * attribute map each have their own attribute map with meta-events. The
 * depth of this recursion is not limited. An attribute map is serialized
 * as follows:
 * <ul>
...
public class XSequentialEventBuffer implements Cloneable {
	...
	private int size = 0;
	/**
	 * The current logical index (in number of events)
	 * in the buffer.
	 */
	private int index = 0;
	/**
	...
	/**
	 * The position at which the last event entry was inserted
	 ...
	 * @param attributeMapSerializer Attribute map serializer to be used.
	 * @throws IOException 
	 */
	 ...
	 * 
	 * @param event The new event to be inserted.
	 * @param index Index to replace at.
	 * @return The former event, having been replaced.
	 */
	public synchronized boolean replace(XEvent event, int index) throws IOException {
	 ...
	 * 
	 * @param eventIndex Position of the requested event, defined
	 * to be within <code>[0, size()-1]</code>.
	 * @return The requested event.
	 */
	public synchronized XEvent get(int eventIndex) throws IOException, IndexOutOfBoundsException {
		// check for index sanity
	 ...
	 * 
	 * @param eventsToSkip Number of records to be skipped.
	 */
	protected synchronized void skipForward(int eventsToSkip) throws IOException {
		int offset = 0;
	 ...
	 * @param eventsToSkip Number of records to be skipped.
	 */
	protected synchronized void skipBackward(int eventsToSkip) throws IOException {
		int offset = 0;
		for(int i=0; i<eventsToSkip; i++) {
	...
	/**
	 * Reads an event from the current position of the data
	...
	protected synchronized XEvent read() throws IOException {
		...
		DataInputStream dis = new DataInputStream(new ByteArrayInputStream(eventData));
